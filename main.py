...
lp_detector = LicensePlate()
writer = None

lp_res={}

def get_license_plate(img):
    lp_text = lp_detector.predict_single(img)
    return lp_text
...
def VisTracking(img, data_track, labels):
    ...
    for i in range(len(data_track)):
        ...
        if(cls_id in [2,3,5,7]):
            if track_id not in lp_res:
                lp_res[track_id]={"text":"","size":0}
            
            if((x1-x0)*(y1-y0)> lp_res[track_id]["size"] or lp_res[track_id]["text"]==""):
                lp_text = get_license_plate(img[y0:y1,x0:x1].copy())
                lp_res[track_id]["text"]=lp_text
                lp_res[track_id]["size"]=(x1-x0)*(y1-y0)
            else:
                lp_text=lp_res[track_id]["text"]
                



        color = (_COLORS[4] * 255).astype(np.uint8).tolist()
        text = labels[cls_id]+"_"+str(track_id)
        if(cls_id in [2,3,5,7]):
            text +=" lp : {}".format(lp_text)
        ...
        cv2.putText(
            img, text, (x0, y0 + txt_size[1]), font, 0.5, txt_color, thickness=1)
...
def ProcessTracking(video, detector, tracker, deep=False, skip_frame=1):
    ...
    writer = cv2.VideoWriter('outpy.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (width,height))
    ...
    while True:
        ...
        if(frame_id % skip_frame == 0):
            ...
            img_draw=VisTracking(frame.copy(), data_track, labels=detector.names)
...
if __name__ == "__main__":
    ...
    video = cv2.VideoCapture("/home/noinoi/video.mp4")
